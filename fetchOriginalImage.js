// 1.3 Duplicate Original Image Plugin (stable fallback - working version)
...
window.addEventListener('load', () => {
  ...
  if (!sdk || typeof sdk.getInfo !== 'function') {
    console.error('[Duplicate Image Plugin] SDK not available.');
    alert('Plugin SDK failed to load. Please refresh the page.');
    return;
  }
  ...
  sdk.getInfo().then((info) => {
    ...
    console.error('[Duplicate Image Plugin] getInfo() failed:', err);
    alert('Failed to initialize plugin. Check console.');
  });
});
