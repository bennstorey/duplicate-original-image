(function () {
  console.log("✅ A20 Plugin: Duplicate Original Image - Dossier Toolbar");
  ...
  waitForContentStationSdk(function () {
    ...
    // Fallback for DOM-based event attachment
    function attachHandlerWithRetry(retries = 30) {
      ...
      if (button) {
        ...
        console.log(`⏳ Waiting for button in DOM (retry ${31 - retries}/30)...`);
        setTimeout(() => attachHandlerWithRetry(retries - 1), 500);
      } else {
        console.warn("❌ Could not find button in DOM after retrying");
      }
    }

    // Use retry logic after button is registered
    attachHandlerWithRetry();
  });
})();
